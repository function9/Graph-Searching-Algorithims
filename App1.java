...
class App1
{
	...
	theGraph.plot("GRAY");
        ...
        // Perform depth first searches and create a new graph 
	String node;
	while (true){
	    System.out.println("\nDFS: Enter the starting node number to start the search (press return to stop): ");
	    node=easy.readString();
	    if (node.length()==0) System.exit(0); // Leave the code
	    Graph dfsPath=theGraph.dfs(Integer.parseInt(node));
	    dfsPath.displayInfoGraph(); //Display the graph info
